...
const UserDashboard: React.FC = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      //               <div>
      //                 <h4 className="font-semibold text-gray-900">{"ORD-" + order._id.toString().slice(-6).toUpperCase()}</h4>
      //                 <p className="text-sm text-gray-600">Placed on {formatDate(order.date)}</p>
      //               </div>
      //               <span className={`px-3 py-1 rounded-full text-sm font-medium ${status_color[order.status.toLowerCase()]}`}>
      //                 {order.status}
      //               </span>
      //             </div>
      //             <div className="flex items-center justify-between">
